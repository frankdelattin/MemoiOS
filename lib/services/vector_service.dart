...
class VectorService {
  ...
  /// Cleans up orphaned vectors for images that were deleted from the gallery
  /// This method should be called during app startup to maintain data consistency
  Future<void> cleanupOrphanedVectors() async {
    try {
      print('Starting orphaned vector cleanup...');
      
      // Get all vector imageIds from database
      final vectorImageIds = await _imageVectorsRepository.getAllImageIds();
      
      if (vectorImageIds.isEmpty) {
        print('No vectors found in database, skipping cleanup');
        return;
      }
      
      // Get current gallery imageIds via photo_manager
      final galleryImageIds = await _imageService.getAllImageIds();
      
      // Find orphaned vectors (in DB but not in gallery)
      final orphanedIds = vectorImageIds.where(
        (id) => !galleryImageIds.contains(id)
      ).toSet();
      
      if (orphanedIds.isNotEmpty) {
        await _imageVectorsRepository.deleteAllByImageIds(orphanedIds);
        print('Cleaned up ${orphanedIds.length} orphaned vectors');
      } else {
        print('No orphaned vectors found');
      }
    } catch (e) {
      print('Error during orphaned vector cleanup: $e');
      // Don\'t rethrow - startup should continue even if cleanup fails
    }
  }
}
